...
- 지도 페이지의 **🍊 소원 지도 ↔ 🍽️ 맛집 지도** 스위치로 전환
- 카테고리: 한식 / 중식 / 일식 / 양식 + 제주 특화 **흑돼지 / 횟집 / 갈치집**
- 네이버 지도·인스타그램에서 유명한 **기본 맛집 21곳**이 지역별로 미리 들어 있음 (`script.js`의 `SEED_FOODS`)
- 소원처럼 **맛집도 직접 추가** 가능 → 스프레드시트의 `맛집` 시트에 저장
  (서버 저장이 안 되면 자동으로 **내 브라우저(localStorage)에만** 저장돼 화면에는 그대로 보입니다)

### 🖼️ 지역별 배경 일러스트 (v2)

지역 페이지마다 시작 페이지와 같은 손그림 컨셉의 전체 화면 일러스트(`assets/region-*.svg`)가 깔립니다.
...
| 추가한 맛집이 다른 기기에서 안 보임 | 최신 `Code.gs`를 붙여넣고 **새 버전으로 재배포**했는지 확인. 구버전 서버면 맛집이 내 브라우저에만 저장됩니다 |

> 🍽️ **맛집 지도 업그레이드 시 할 일**: 최신 `Code.gs`를 Apps Script에 다시 붙여넣고
> **배포 → 배포 관리 → 새 버전**으로 재배포하면 끝. `맛집` 시트는 첫 저장 때 자동으로 생성됩니다.
> (컬럼: `타임스탬프 | 지역 | 카테고리 | 가게이름 | 소개`)
